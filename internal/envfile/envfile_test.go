...
func TestParse(t *testing.T) {
	tests := []struct {
		...
		{"multi-line double quoted", "A=\"line one\nline two\"\nB=1\n", map[string]string{"A": "line one\nline two", "B": "1"}},
		{"multi-line single quoted", "A='one\ntwo\nthree'\n", map[string]string{"A": "one\ntwo\nthree"}},
		{"multi-line crlf normalized to lf", "A=\"one\r\ntwo\"\r\nB=2\r\n", map[string]string{"A": "one\ntwo", "B": "2"}},
		{"multi-line keeps comments and blank lines", "A=\"one\n# not a comment\n\ntwo\"\n", map[string]string{"A": "one\n# not a comment\n\ntwo"}},
		{"multi-line with escapes", "A=\"say \\\"hi\\\"\nbye\"\n", map[string]string{"A": "say \"hi\"\nbye"}},
		{"multi-line keeps trailing spaces on the opening line", "A=\"abc   \ndef\"\n", map[string]string{"A": "abc   \ndef"}},
		{"multi-line followed by comment", "A=\"x\ny\" # c\nB='p\nq'\t#c\n", map[string]string{"A": "x\ny", "B": "p\nq"}},
		{"pem style", "KEY=\"-----BEGIN KEY-----\nabc\n-----END KEY-----\"\n", map[string]string{"KEY": "-----BEGIN KEY-----\nabc\n-----END KEY-----"}},
...
func TestParseErrors(t *testing.T) {
	tests := []struct {
		...
		{"unterminated across lines", "A=\"" + secret + "\nmore\n", 1, "unterminated double-quoted value"},
		{"unterminated single across lines", "A='" + secret + "\nmore\n", 1, "unterminated single-quoted value"},
		{"escaped closing quote", `A="` + secret + `\"`, 1, "unterminated double-quoted value"},
		{"text after multi-line closing quote", "A='x\ny'" + secret, 1, "unexpected text after closing quote"},
		{"duplicate after multi-line value", "A=\"x\ny\nz\"\nB=1\nA=" + secret, 5, `duplicate key "A" (already defined at line 1)`},
