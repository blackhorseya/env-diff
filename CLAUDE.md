...
- **Output layout is golden-tested** in `presenter_test.go` and `cli_test.go`, and README samples are pasted from real output. Color must never change layout: `TestTerminalColor` strips the ANSI codes and expects byte equality with plain output. JSON lists are always arrays (`orEmpty`), never `null`.
